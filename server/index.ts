...
app.post("/certificates", authenticated, allow("admin", "uploader"), async (req, res) => {
  ...
  const record = { certificateId, documentHash: documentHash.toLowerCase(), cid, recipientName, recipientEmail: safe(req.body.recipientEmail), credential, organization, issueDate: safe(req.body.issueDate, 20), expiryDate: safe(req.body.expiryDate, 20), description: safe(req.body.description, 1500), issuerWallet: issuerWallet.toLowerCase(), transactionHash, blockNumber: safe(req.body.blockNumber, 30), status: "valid", uploaderUid: req.user!.uid, createdAt: new Date().toISOString() };
  try { const { db } = firebase(); await db.collection("certificates").doc(certificateId).create(record); await db.collection("activity").add({ type: "certificate_issued", certificateId, by: req.user!.uid, createdAt: FieldValue.serverTimestamp() }); res.status(201).json({ certificate: record }); }
  catch (error) { res.status(409).json({ error: error instanceof Error && /already exists/i.test(error.message) ? "Certificate ID already exists." : "Could not save certificate metadata." }); }
});
